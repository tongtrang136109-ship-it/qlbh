        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 z-[60] flex justify-center items-center p-4">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-lg">
                <div className="p-4 border-b flex justify-between items-center">
                    <h3 className="text-xl font-bold text-slate-800">Thêm liên hệ</h3>
                ...
                <div className="p-6 space-y-4">
                    <div>
                        ...
                        <input type="text" value={name} onChange={e => setName(e.target.value)} className="mt-1 w-full p-2 border rounded-md" autoFocus />
                    </div>
                     <div>
                        <label className="block text-sm font-medium text-slate-700">Điện thoại</label>
                        <input type="text" value={phone} onChange={e => setPhone(e.target.value)} className="mt-1 w-full p-2 border rounded-md" />
                    </div>
                    <div>
            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-xl max-h-[95vh] flex flex-col">
                ...
                <div className="p-6 space-y-4 overflow-y-auto">
                    <div className="relative">
                        <label className="block text-sm font-medium text-slate-700">{contactLabel} <span className="text-red-500">*</span></label>
                        <div className="flex items-center mt-1">
                            ...
                            />
                            <button onClick={onAddNewContact} className="p-2 border-t border-b border-r rounded-r-md h-[42px] bg-slate-50 hover:bg-slate-100"><PlusIcon /></button>
                        </div>
                        {isContactListOpen && (
                        ...
                        {selectedContact && <div className="mt-2 p-2 bg-sky-50 border border-sky-200 rounded-md text-sky-800 font-semibold">{selectedContact.name}</div>}
                    </div>
                     <div>
                        <label className="block text-sm font-medium text-slate-700">Số tiền thanh toán</label>
                        <input type="number" placeholder="0" value={amount} onChange={e => setAmount(Number(e.target.value))} className="mt-1 w-full p-2 border rounded-md"/>
                    </div>
                     <div>
                        <label className="block text-sm font-medium text-slate-700">Nội dung ghi chú</label>
                        <textarea rows={3} value={notes} onChange={e => setNotes(e.target.value)} className="mt-1 w-full p-2 border rounded-md"></textarea>
                    </div>
                    <div>
                        <div className="flex justify-between items-center">
                            <label className="block text-sm font-medium text-slate-700">Hình thức thanh toán</label>
                            <button onClick={onConfigureSources} className="text-xs text-orange-600 hover:underline flex items-center"><Cog6ToothIcon className="w-4 h-4 mr-1"/> Thiết lập nguồn tiền</button>
                        </div>
                        <div className="flex flex-wrap gap-2 mt-2">
                           {paymentSources.map(ps => (
                               <button key={ps.id} onClick={() => setSelectedPaymentSourceId(ps.id)} className={`p-3 border rounded-lg text-left ${selectedPaymentSourceId === ps.id ? 'border-sky-500 bg-sky-50 ring-2 ring-sky-300' : 'border-slate-300 bg-white'}`}>
                                   <p className="font-semibold">{ps.name}</p>
                                   <p className="text-sm text-slate-600">{formatCurrency(ps.balance)}</p>
                               </button>
                           ))}
                        </div>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-slate-700">Thời gian thanh toán</label>
                        ...
                        {!useCurrentTime && <input type="datetime-local" value={customDate} onChange={e => setCustomDate(e.target.value)} className="mt-2 p-2 border rounded-md w-full sm:w-auto"/>}
                        {useCurrentTime && <p className="text-sm text-slate-500 mt-2 p-2 border rounded-md bg-slate-50 inline-block">{new Date().toLocaleString('vi-VN')}</p>}
                    </div>
                </div>
                <div className="p-4 bg-slate-50 border-t flex justify-end">
                    <button onClick={handleSave} className="bg-orange-500 text-white font-bold py-2 px-6 rounded-lg hover:bg-orange-600">LƯU</button>
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
            <h1 className="text-3xl font-bold text-slate-800">Quản lý Thu Chi</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white p-5 rounded-xl shadow-sm flex items-center border"><div className="p-4 rounded-full bg-green-100"><ArrowUpCircleIcon className="w-7 h-7 text-green-600" /></div><div className="ml-4"><p className="text-sm text-slate-500">Tổng thu trong kỳ</p><p className="text-2xl font-bold text-green-600">{formatCurrency(summary.totalIncome)}</p></div></div>
            <div className="bg-white p-5 rounded-xl shadow-sm flex items-center border"><div className="p-4 rounded-full bg-red-100"><ArrowDownCircleIcon className="w-7 h-7 text-red-600" /></div><div className="ml-4"><p className="text-sm text-slate-500">Tổng chi trong kỳ</p><p className="text-2xl font-bold text-red-600">{formatCurrency(summary.totalExpense)}</p></div></div>
        </div>
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200/60">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
                <div>
                    ...
                    <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="mt-1 block w-full p-2 border border-slate-300 rounded-md shadow-sm" />
                </div>
                <div>
                    <label className="block text-sm font-medium text-slate-700">Đến ngày</label>
                    <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="mt-1 block w-full p-2 border border-slate-300 rounded-md shadow-sm" />
                </div>
                <div className="lg:col-span-3 flex flex-wrap items-end gap-2">
                    <button onClick={handleTodayClick} className="px-3 py-2 border rounded-md text-sm font-medium bg-slate-100 hover:bg-slate-200">Hôm nay</button>
                    <button onClick={handleThisWeekClick} className="px-3 py-2 border rounded-md text-sm font-medium bg-slate-100 hover:bg-slate-200">Tuần này</button>
                    <button onClick={handleThisMonthClick} className="px-3 py-2 border rounded-md text-sm font-medium bg-slate-100 hover:bg-slate-200">Tháng này</button>
                    <button onClick={handleThisYearClick} className="px-3 py-2 border rounded-md text-sm font-medium bg-slate-100 hover:bg-slate-200">Năm nay</button>
                    <button onClick={() => {setStartDate(''); setEndDate('');}} className="px-3 py-2 border rounded-md text-sm font-medium bg-slate-100 hover:bg-slate-200 text-red-600">Bỏ lọc</button>
                </div>
            </div>
        </div>
            ...
            <table className="w-full text-left">
                <thead className="border-b bg-slate-50">
                    <tr>
                        ...
                        <th className="p-3 font-semibold text-slate-600">Đối tác</th>
                        <th className="p-3 font-semibold text-slate-600">Nội dung</th>
                        <th className="p-3 font-semibold text-slate-600 text-right">Đã thu</th>
                        <th className="p-3 font-semibold text-slate-600 text-right">Đã chi</th>
                        <th className="p-3 font-semibold text-slate-600">Nguồn tiền</th>
                    </tr>
                </thead>
                <tbody>
                    {filteredTransactions.length > 0 ? filteredTransactions.map(tx => {
                        ...
                        return (
                        <tr key={tx.id} className="border-b hover:bg-slate-50 text-sm">
                            <td className="p-3">{new Date(tx.date).toLocaleString('vi-VN')}</td>
                            <td className="p-3 font-medium text-slate-800">{tx.contact.name}</td>
                            <td className="p-3 text-slate-600">{tx.notes}</td>
                            <td className="p-3 text-right font-semibold text-green-600">{tx.type === 'income' ? formatCurrency(tx.amount) : ''}</td>
                            <td className="p-3 text-right font-semibold text-red-600">{tx.type === 'expense' ? formatCurrency(tx.amount) : ''}</td>
                            <td className="p-3">{source?.name || 'N/A'}</td>
                        </tr>
                    )}) : (
                         <tr>
